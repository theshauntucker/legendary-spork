...
import { Trophy, Star, TrendingUp, Calendar, ChevronRight, Award, Sparkles } from "lucide-react";
...
function scoreBar(score: number, max = 300) {
...
export default async function DancersPage() {
  ...
  // Fetch all analyzed videos with their analyses for this user
  ...
  for (const video of videos ?? []) {
    const name = video.dancer_name || "Unnamed Dancer";
    ...
    if (!dancerMap.has(name)) {
      dancerMap.set(name, {
        ...
        bestAward: award,
        latestAward: award,
        latestCompetition: video.competition_name || null,
        latestDate: video.competition_date || video.created_at,
        styles: [],
        scoreHistory: [],
      });
    }
    ...
    if (video.style && !dancer.styles.includes(video.style)) {
      dancer.styles.push(video.style);
    }
  }

  const dancers = Array.from(dancerMap.values()).sort(
    (a, b) => b.totalAnalyses - a.totalAnalyses
  );
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-10 mt-6">
          <div className="flex items-center gap-3 mb-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-gold-500/30 to-accent-500/30">
          ...
          <p className="text-surface-200 ml-[52px]">
        ...
        {dancers.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
            {[
              { label: "Dancers Tracked", value: dancers.length, icon: Star },
              { label: "Total Analyses", value: totalAnalyses, icon: BarChart },
              { label: "Best Score", value: topScore > 0 ? `${topScore}` : "—", icon: Trophy },
              { label: "Styles Covered", value: [...new Set(dancers.flatMap(d => d.styles))].length, icon: Award },
            ].map((stat) => (
              <div key={stat.label} className="glass rounded-2xl p-5 text-center">
                <stat.icon className="mx-auto h-5 w-5 text-gold-400 mb-2" />
        ...
        {dancers.length === 0 ? (
          <div className="glass rounded-3xl p-16 text-center">
            ...
            <p className="text-surface-200 mb-6 max-w-md mx-auto">
              Upload your first routine with a dancer name and we&apos;ll track every score, every award, and every improvement across the whole season.
            </p>
            <Link
              href="/upload"
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-1">
                        <h3 className="text-xl font-bold group-hover:text-primary-300 transition-colors truncate">
                          {dancer.name}
                        </h3>
                        <span className={`shrink-0 inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-full border ${awardColor(dancer.bestAward)}`}>
                          {awardEmoji(dancer.bestAward)} {dancer.bestAward}
                        </span>
                      </div>

                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-surface-200 mb-3">
                        {dancer.studio && <span>{dancer.studio}</span>}
                        {dancer.studio && dancer.ageGroup && <span className="text-white/20">·</span>}
                    ...
                    <div className="shrink-0 text-right">
                      <div className="flex items-center gap-1.5 justify-end text-surface-200 mb-2">
                        <Calendar className="h-3.5 w-3.5" />
                        <span className="text-xs">
                          {dancer.totalAnalyses} {dancer.totalAnalyses === 1 ? "analysis" : "analyses"}
                        </span>
                      </div>
                      ...
                      {dancer.latestCompetition && (
                        <p className="text-xs text-surface-200 mt-1 max-w-[140px] text-right truncate">
                          {dancer.latestCompetition}
                        </p>
                      )}

                      <ChevronRight className="h-5 w-5 text-surface-200 group-hover:text-white group-hover:translate-x-1 transition-all mt-2 ml-auto" />
                    </div>
                  </div>
                </Link>
              );
        ...
        {dancers.length > 0 && (
          <div className="mt-8 text-center">
            <Link
              href="/upload"
              className="inline-flex items-center gap-2 rounded-full border border-white/20 px-6 py-3 text-sm font-semibold hover:bg-white/5 transition-colors"
            >

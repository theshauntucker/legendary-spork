...
export async function POST(request: NextRequest) {
  try {
    ...
    const granted = await grantFreeCreditIfNew(serviceClient, user.id, user.email);
    return NextResponse.json({ success: true, granted: granted ? 1 : 0, alreadyGranted: !granted });
  } catch (err) {
